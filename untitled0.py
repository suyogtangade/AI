...
P=np.transpose(np.linalg.solve(np.block([[np.transpose(np.subtract(A,B))], [np.transpose(np.subtract(B,C))]]), np.block([[(np.square(np.linalg.norm(A))-np.square(np.linalg.norm(B)))/2], [(np.square(np.linalg.norm(B))-np.square(np.linalg.norm(C)))/2]])))
print(P)
print(A)
print(B)
print(C)
